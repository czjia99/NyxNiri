...
### Added
- Niri 视觉特效零件新增 xray-blur 预设，采用背景预模糊降低半透明窗口开销
- 常用浏览器支持按需开启可变刷新率（VRR），减少滚动与视频播放时的画面撕裂
- 新增 `nyxuri shell` 命令，可查看当前桌面外壳状态并切换为自研或自定义外壳
- Kitty 支持鼠标右键直接粘贴剪贴板，关闭划词自动复制避免剪贴板被污染
- Kitty 终端 Ctrl+C 智能识别：有划选内容时复制并清除高亮，无选区时发送中断信号
- 锁屏状态下支持直接调节多媒体音量与静音
- 预设管理界面支持双栏切换视觉特效与发光边框等独立零件，无需重载全部配置
...
- 缓存清理统一为 `nyxuri clean`，原来的 Fish 和 Orbit 入口仍可用
- 安装输入法不再顺带启用皮肤；皮肤切换不改快捷键、不重启输入法，卸载保留个人改动
- 安装 Fcitx5-Rime 时自动配置好雾凇拼音方案，无需手动初始化
- Niri 快捷键改由网关脚本分发，未安装星环启动器时自动回退到备选启动器
- 壁纸选择器与星环启动器直接读取系统调色板，色彩表现更一致
- 主题深浅切换统一由 `nyxuri theme` 完成
- 预设改用独立存储目录，不再与软件默认文件冲突
- 卸载壁纸时只移除项目自带素材，不再触碰用户的私人壁纸目录
- 启动时若自定义桌面外壳异常，自动弹出桌面通知并回退到 Noctalia 避免黑屏

### Fixed
- 修复重新应用配置后 Niri 窗口特效零件被重置为默认模糊的问题
- 修复初次部署后默认窗口边框被发光特效覆盖的问题，发光效果现作为独立零件供按需开启
- 修复深浅模式切换时输入法皮肤未动态跟随的问题，并防止未启动时意外唤醒后台进程
- 修复登录界面同步外观时误弹窗口索要管理员密码的问题
- 修复更新操作在自动确认模式下未创建备份快照的问题，现在更新前均会自动生成受保快照
...
[Unreleased]: https://github.com/ech678/Nyxuri/compare/v3.0.5...HEAD
