...
from nyxuri.core import get_env, log_msg, register_temp_path
from nyxuri.i18n import msg, text
from nyxuri.network import fetch_raw_with_fallback
...
def fisher_installed() -> bool:
    """True only when Nyxuri has a valid Fisher ownership record."""
...
def fisher_install() -> bool:
    ...
    if state and state["fisher_preexisting"]:
        log_msg("INFO", "Fisher predated Nyxuri ownership; skipped")
        return False
    ...
    if state is None and (fisher_file.is_file() or fisher_file.is_symlink()):
        log_msg("INFO", "Fisher already exists without Nyxuri ownership; skipped")
        return False
...
def fisher_uninstall() -> bool:
    """Remove exactly the Fisher files Nyxuri recorded as its own."""
    state = _load_ownership()
    if state is None:
        return False
    fish_dir = get_env().config_dir / "fish"
    ...
    log_msg("INFO", "Removed Nyxuri-owned Fisher files")
    return True
